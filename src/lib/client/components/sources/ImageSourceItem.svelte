<script lang="ts">
	...
	import ImagePullModal from '$lib/client/components/sources/ImagePullModal.svelte';
...
<div class="card shadow-lg my-4 bg-base-100 p-3 flex flex-row pl-0 h-auto overflow-hidden">
	...
	<a
		class="block w-64 flex-auto overflow-hidden mr-1 sm:mr-3 pr-1 sm:pr-4 hover:text-primary-focus"
	...
	<div class="block w-24 md:w-24 overflow-visible flex-shrink-0 self-center">
		<ActionButton
			icon={DownloadIcon}
			{loading}
			class="w-24 h-8 md:h-12 md:w-24 md:mr-2"
			on:click={() => (open = true)}
		>
			Pull
		</ActionButton>
		...
		<a
			...
			target="_blank"
			rel="noreferrer"
			class="btn btn-ghost w-24 justify-start h-10 btn-block min-h-0 text-base px-2 mt-2"
		>
